...
const somaPagamento = (price, payment) => {
  for (let each in payment) {
    if (price === each) {
      return payment[each];
    }
  }
...
const payLoad = () => {
  let total = 0;
  const objeto = restaurant.fetchMenu();
  const payment = {};
  Object.assign(payment, objeto.food, objeto.drink);
  for (let price of restaurant.consumption) {
